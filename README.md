...
SIが本体。光合成を、ゆらぎのあるエネルギー源 (環境光) からランダムにON/OFFするアンテナ (クロロフィル) でエネルギーを回収しターゲット出力Ωを得るシステムと抽象化。アンテナ1つより2つのほうがノイズの少ない出力を得られる (任意の単一アンテナに対しそのようなツインアンテナが存在する) ことを示した。さまざまな環境光スペクトルを使ってアンテナの最適解を求めることで実際のクロロフィルa/bの吸収ピークを精度よく再現した。
