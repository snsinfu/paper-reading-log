...
CNN ベースの VAE に蛋白質の安定状態の構造を学習させ、その間に存在する未観測の遷移状態を予測。デコーダロスが物理的で、原子の結合長、結合角、LJ 相互作用、クーロン相互作用を取り込んでいる。さらに入力した2つの構造 x1, x2 のラテント表現 z1, z2 の再構築ロスだけでなく、z1, z2 を結ぶ線上でロス関数を積分して使うのでら物理的に妥当な遷移状態が予測できるという触れ込み。
